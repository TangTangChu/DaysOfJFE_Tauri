        ...
        case "choice_show": {
            ...
            if (!Array.isArray(payload.options)) {
                ...
                payload.options.forEach((opt, i) => {
                    if (!isRecord(opt)) {
                        errors.push(
                            makeError(`${path}.options[${i}] 必须是对象`),
                        );
                        return;
                    }
                    ...
                    if (typeof opt.id === "string") {
                        if (ids.has(opt.id)) {
                            errors.push(
                                makeError(`${path}.options[${i}].id 重复`, {
                                    id: opt.id,
                                }),
                            );
                        }
    ...
    document.scenes.forEach((scene, sceneIndex) => {
        ...
        scene.commands.forEach((command, commandIndex) => {
            ...
            if (!command.id) {
                ...
                errors.push(
                    makeError(`${commandPath}.id 格式非法`, { id: command.id }),
                );
            }

            if (commandIdSet.has(command.id)) {
                errors.push(
                    makeError(`${commandPath}.id 重复`, { id: command.id }),
                );
            }
