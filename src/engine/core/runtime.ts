...
export class Runtime {
    ...
    async dispatch(action: RuntimeAction): Promise<void> {
        switch (action.type) {
            ...
            case "system.reset":
                ...
                if (this.ir) {
                    this.state.variables = structuredClone(
                        this.ir.initialVariables,
                    );
    ...
    private registerDefaultHandlers(): void {
        ...
        this.bus.register("bg_set", (command, ctx) => {
            const context = ctx;
            context.state.stage.backgroundKey = String(
                command.payload.bgKey ?? "",
            );
        });
        ...
        this.bus.register("emit", (command, ctx) => {
            const context = ctx;
            const event = String(
                command.payload.event ?? "engine.state.changed",
            );
            context.emitEvent(
