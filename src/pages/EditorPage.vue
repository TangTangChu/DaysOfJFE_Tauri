...
const sceneCommandCount = computed(() => {
    ...
    return doc.value.scenes.reduce(
...
const activeCommand = computed(() => {
    ...
    return (
        structureCommands.value.find(
            (cmd) => cmd.id === activeCommandId.value,
...
const assetLibrary = computed<Record<AssetCategory, AssetLibraryEntry[]>>(
    () => {
        ...
        return {
            ...
            standees: Object.entries(assets.standees ?? {}).map(
                ([id, src]) => ({
                    id,
                    src,
                }),
            ),
            bgm: Object.entries(assets.bgm ?? {}).map(([id, src]) => ({
                id,
                src,
            })),
            ...
            sfx: Object.entries(assets.sfx ?? {}).map(([id, src]) => ({
                id,
                src,
            })),
        };
...
function resolveCommandDragPayload(
    ...
    const commandId =
        event.dataTransfer?.getData(COMMAND_DRAG_ID_MIME) ||
        draggedCommandId.value;
    if (commandId) {
...
function reorderCommandToIndex(commandId: string, insertIndex: number) {
    ...
    const boundedIndex = Math.max(
        0,
        Math.min(insertIndex, scene.commands.length),
    );
...
function deleteActiveCommand() {
    ...
    const scene = doc.value.scenes.find((s) => s.id === activeScene.value!.id);
    if (!scene) return;
    const idx = scene.commands.findIndex(
        (c) => c.id === activeCommand.value!.id,
    );
    if (idx < 0) return;
...
function handleEditorDrop(event: DragEvent) {
    ...
    event.preventDefault();
    const snippet =
        event.dataTransfer?.getData("text/plain") || dragSnippet.value;
    if (!snippet) {
        clearSnippetDragState();
        return;
    }
...
async function loadSample(id: string) {
    const path = Object.keys(scriptSources).find((p) =>
        p.endsWith(`/${id}.yaml`),
    );
    if (!path) return;
...
function handleStageDragLeave(event: DragEvent) {
    ...
    if (
        nextTarget instanceof Node &&
        stageFrameRef.value?.contains(nextTarget)
    ) {
        return;
    }
    ...
    >
        ...
        >
            <div class="flex items-center gap-4">
                <h1
                    class="font-serif text-xl font-extrabold text-gal-text tracking-widest m-0 inline-flex items-center gap-2"
                >
                    剧本编辑
                </h1>
        ...
        <div class="flex-1 flex overflow-hidden p-6 gap-8">
            <aside class="w-64 lg:w-72 shrink-0 flex flex-col gap-6 min-h-0">
                <div class="gal-panel p-5 flex flex-col gap-4 shrink-0">
                    <div
                        class="text-[11px] font-bold tracking-widest uppercase text-md-primary/80"
                    >
                        剧本属性
                    </div>
                    <GalField label="ID">
                        <input
                            ...
                            class="gal-input text-xs py-1.5 px-3 border-md-outline-variant focus:border-md-primary transition-all bg-transparent"
                            @blur="applyMeta"
                        />
                    </GalField>
                    <GalField label="标题">
                ...
                <!-- 场景大纲 -->
                <div
                    class="gal-panel flex-1 min-h-0 flex flex-col overflow-hidden"
                >
                    <div class="px-5 py-4 shrink-0">
                        <div
                            class="text-[11px] font-bold tracking-widest uppercase text-md-primary/80 flex justify-between items-center"
                        >
                            <span>场景大纲</span>
                    ...
                    <div
                        class="flex-1 overflow-y-auto px-2 pb-5 flex flex-col gap-1 custom-scrollbar"
                    >
                        <button
                            v-for="scene in sceneList"
                            :key="scene.id"
                            class="w-full text-left rounded-2xl px-4 py-3 transition-colors duration-200 cursor-pointer group"
                            :class="
                                activeScene?.id === scene.id
                                    ? 'bg-md-primary-container text-md-on-primary-container'
                                    : 'bg-transparent text-md-on-surface hover:bg-md-surface-container-highest'
                            "
                        ...
                        >
                            <div class="min-w-0 flex-1">
                                ...
                                <div
                                    class="text-[10px] opacity-50 mt-0.5 font-mono truncate"
                                >
                                    {{ scene.id }}
                                </div>
                            </div>
                ...
                <div class="gal-panel p-5 grid grid-cols-1 gap-4 shrink-0">
                    <div class="flex items-center justify-between">
                        <div
                            class="text-[10px] font-bold tracking-wider text-gal-text-sub uppercase opacity-50"
                        >
                            Commands
                        </div>
                        <div class="text-lg font-black text-md-on-surface">
                            {{ sceneCommandCount }}
                        </div>
                    </div>
                    <div class="flex items-center justify-between">
                        ...
                        <div class="text-lg font-black text-md-on-surface">
                            {{
                                doc
                                    ? Object.keys(doc.variables ?? {}).length
                                    : 0
                            }}
                        </div>
                    </div>
            ...
            <main
                class="flex-1 min-w-0 flex flex-col gap-0 gal-panel overflow-hidden bg-md-surface-container-low border-none"
            >
                <!-- 上下文标题 -->
                <div
                    class="shrink-0 px-6 py-3 flex items-center justify-between bg-transparent border-b border-md-outline-variant/5"
                >
                    <div class="flex items-center gap-3">
                        <span
                            class="text-xs font-bold tracking-widest text-md-on-surface"
                        >
                            {{ activeScene?.title || "未选择场景" }}
                        </span>
                        <span class="text-[9px] font-mono opacity-30">{{
                            activeScene?.id
                        }}</span>
                    </div>
                ...
                <div class="flex-1 min-h-0 overflow-hidden relative">
                    <div v-if="viewMode === 'editor'" class="absolute inset-0">
                        <textarea
                            ref="editorRef"
                            v-model="rawText"
                            class="w-full h-full p-6 font-sans text-[13px] tracking-widest resize-none bg-transparent outline-none text-md-on-surface custom-scrollbar"
                            placeholder="在这里编写 YAML 剧本..."
                            @dragover="handleEditorDragOver"
                            @drop="handleEditorDrop"
                        ></textarea>
                        <div
                            class="absolute right-6 bottom-6 z-10 text-[10px] text-gal-text-sub tracking-widest pointer-events-none opacity-50 bg-md-surface-container px-3 py-1.5 rounded-full"
                        >
                            拖入积木可快速插入
                        </div>
                    </div>
                    ...
                    >
                        <div
                            class="w-full xl:w-80 flex flex-col shrink-0 h-1/2 xl:h-auto border-r border-md-outline-variant/10"
                        >
                            <div class="p-5 shrink-0 flex flex-col gap-4">
                                <button
                                    class="w-full rounded-2xl px-4 py-3 text-center text-sm font-bold tracking-widest bg-md-primary text-white hover:bg-md-primary/90 transition-all cursor-pointer active:scale-95"
                                ...
                                <div class="relative">
                                    <input
                                        v-model="searchText"
                                        class="gal-input text-xs py-2 px-3 focus:border-md-primary transition-all bg-transparent"
                                        placeholder="搜索指令..."
                                    />
                                </div>
                                <GalDropdown
                            ...
                            <div
                                ref="commandListRef"
                                class="flex-1 overflow-y-auto px-4 pb-5 flex flex-col gap-2 custom-scrollbar"
                            ...
                            >
                                ...
                                >
                                        ...
                                        :class="
                                            dragInsertIndex === index
                                                ...
                                                : inlineInsertIndex === index
                                                  ...
                                                  : 'bg-transparent hover:bg-md-surface-container-highest/50'
                                        "
                                        @dragover="
                                    ...
                                    >
                                        <button
                                            class="w-full px-3 py-1.5 text-[10px] font-bold text-center text-md-primary cursor-pointer opacity-40 hover:opacity-100 transition-opacity"
                                            @click="openInlineInsert(index)"
                                        >
                                            + 插入
                                        </button>
                                        <div
                                            v-if="inlineInsertIndex === index"
                                            class="px-3 pb-3"
                                        >
                                            <GalDropdown
                                                :model-value="''"
                                    ...
                                    <div
                                        ...
                                        class="rounded-2xl px-4 py-3.5 cursor-pointer transition-all duration-200 border-none relative overflow-hidden group"
                                        :class="
                                            activeCommand?.id === command.id
                                                ? 'bg-md-primary-container text-md-on-primary-container'
                                                : draggedCommandId ===
                                                    command.id
                                                  ? 'bg-md-surface-container-highest opacity-50 scale-95'
                                        ...
                                        @dragover="
                                            handleCommandCardDragOver(
                                                index,
                                                $event,
                                            )
                                        "
                                        @drop="
                                            handleCommandCardDrop(index, $event)
                                        "
                                    >
                                        <div class="flex items-start gap-3">
                                            <div
                                                class="mt-0.5 shrink-0 w-5 h-5 flex items-center justify-center rounded bg-black/5 text-current/30 group-hover:text-current/60 transition-colors cursor-grab active:cursor-grabbing"
                                                draggable="true"
                                                @click.stop
                                                @dragstart.stop="
                                                    handleCommandDragStart(
                                            ...
                                            <div class="min-w-0 flex-1">
                                                ...
                                                >
                                                    ...
                                                    <span
                                                        class="ml-auto text-[9px] font-mono opacity-30"
                                                        >#{{ index + 1 }}</span
                                                    >
                                                </div>
                                                <div
                                                    class="text-[13px] font-medium leading-normal tracking-wide line-clamp-2 opacity-90"
                                                >
                                                    {{
                                ...
                                <div
                                    v-if="!isCommandListFiltered"
                                    class="rounded-xl transition-colors bg-transparent hover:bg-md-surface-container-highest/50"
                                    :class="
                                        dragInsertIndex ===
                                        structureCommands.length
                                            ? 'bg-md-secondary-container opacity-100'
                                            : inlineInsertIndex ===
                                                structureCommands.length
                                              ? 'bg-md-surface-container-highest opacity-100'
                                              : ''
                                    "
                                    @dragover="
                                        handleCommandInsertZoneDragOver(
                                ...
                                <div
                                    ...
                                    class="rounded-2xl bg-md-surface-container px-6 py-10 text-center text-xs font-medium text-md-on-surface-variant opacity-50 italic"
                                >
                                    {{
                        ...
                        <div
                            class="flex-1 overflow-y-auto bg-transparent custom-scrollbar h-1/2 xl:h-auto"
                        >
                            <div
                                v-if="activeCommand"
                                class="flex flex-col gap-6 max-w-3xl mx-auto p-6 lg:p-10"
                            >
                                <div
                                    class="flex items-center justify-between gap-6 pb-4 border-b border-md-outline-variant/10"
                                >
                                    <div class="flex items-center gap-3">
                                        <span
                                            class="px-2.5 py-1 rounded-lg bg-md-primary-container text-md-on-primary-container text-[10px] font-bold tracking-widest uppercase"
                                        >
                                            {{ typeLabel(activeCommand.type) }}
                                        </span>
                                        <span
                                            class="text-[10px] text-gal-text-light font-mono opacity-40 px-2 py-0.5 rounded bg-md-surface-container"
                                        >
                                            {{ activeCommand.id }}
                                        </span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <button
                                            class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-md-surface-container-highest text-md-on-surface-variant cursor-pointer hover:bg-md-primary-container hover:text-md-on-primary-container transition-colors active:scale-95 text-xs"
                                            title="上移"
                                        ...
                                        <div
                                            class="w-px h-4 bg-md-outline-variant/20 mx-1"
                                        ></div>
                                        <button
                                            class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-transparent text-red-400 cursor-pointer hover:bg-red-50 hover:text-red-500 transition-colors active:scale-90 text-xs"
                                            title="删除"
                                            @click="
                                                showConfirm(
                                                    '删除指令',
                            ...
                            <div
                                v-else
                                class="h-full flex flex-col items-center justify-center text-sm font-bold tracking-widest text-gal-text-sub opacity-20 uppercase select-none"
                            >
                                Select an command to configure
                            </div>
                        </div>
                    </div>
                    ...
                    >
                        <div class="grid grid-cols-3 gap-4 mb-6 shrink-0">
                            <div class="gal-panel p-4">
                                <div class="text-xs text-gal-text-sub">
                                    解析错误
                                </div>
                        ...
                        <div class="flex flex-col gap-3">
                            ...
                            >
                                ...
                                <div
                                    class="text-sm font-semibold mt-1 text-red-200"
                                >
                                    {{ err.message }}
                                </div>
                                <pre
            ...
            <aside
                class="w-80 lg:w-100 xl:w-120 shrink-0 flex flex-col gap-4 min-h-0"
            >
                <div
                    class="w-full aspect-video rounded-3xl bg-[#0a0a0a] relative overflow-hidden shrink-0"
                >
                    <div
                        v-if="!previewReady"
                    ...
                    <template v-if="previewReady">
                        ...
                        <DialogueBox
                            ...
                            :speaker-name="
                                previewDialogue?.speakerName ?? undefined
                            "
                            :display-text="
                                previewDialogue?.textRendered || '...'
                            "
                            :is-typing="
                                Boolean(
                ...
                <div
                    class="shrink-0 flex items-center justify-between gap-2 p-2 rounded-3xl bg-md-surface-container"
                >
                    <button
                        class="flex-1 py-1.5 text-xs font-semibold rounded-xl hover:bg-md-surface-container-high transition-colors cursor-pointer flex items-center justify-center gap-1"
                        :class="
                            previewReady ? 'text-green-400' : 'text-md-primary'
                        "
                        @click="syncPreview"
                    >
                    ...
                    <button
                        class="flex-1 py-1.5 text-xs font-semibold rounded-xl hover:bg-md-surface-container-high transition-colors cursor-pointer text-md-on-surface-variant"
                    ...
                    <button
                        class="flex-1 py-1.5 text-xs font-semibold rounded-xl hover:bg-md-surface-container-high transition-colors cursor-pointer text-md-primary"
                ...
                <div
                    class="flex-1 min-h-0 overflow-hidden flex flex-col rounded-[28px] bg-md-surface-container"
                >
                    <div class="shrink-0 p-4 pb-0">
                        <span
                            class="text-sm font-semibold tracking-widest text-md-on-surface"
                            >创作与配置面板</span
                        >
                    </div>
                    ...
                    >
                        ...
                        <div class="flex flex-col gap-3 px-1 py-1">
                            <div
                                class="text-[11px] text-md-primary uppercase tracking-widest font-semibold"
                            >
                                快速插入（拖拽至左侧）
                            </div>
                            <div class="flex flex-wrap gap-2">
                                <button
                                    ...
                                    :key="item.id"
                                    draggable="true"
                                    class="rounded-full bg-md-surface-container-high px-3 py-1.5 text-xs font-medium text-md-on-surface cursor-grab active:cursor-grabbing hover:bg-md-secondary-container hover:text-md-on-secondary-container transition-colors"
                                    @click="
                                        handleQuickInsert(
                                            item.id as ScriptCommandType,
                                        )
                                    "
                                    @dragstart="
            ...
            >
                ...
                >
                    <div class="flex items-center gap-3">
                        ...
                        />
                        <span
                            class="text-sm text-white/80 font-semibold tracking-wider"
                            >{{ activeCommand?.id ?? "—" }}</span
                        >
                    </div>
                    <div class="flex items-center gap-2">
                        <button
                ...
                <div class="flex-1 flex overflow-hidden">
                    ...
                    >
                        ...
                        >
                                ...
                                <template v-if="previewStage?.characters">
                                    <div
                                        ...
                                        :class="{
                                            ...
                                            'right-[10%]':
                                                char.slot === 'right',
                                            'ring-2 ring-md-primary ring-offset-2 ring-offset-black/40 rounded-3xl':
                                                activeCharacterId ===
                                                String(id),
                                        }"
                                        draggable="true"
                                        @dragstart="
                                            handleStageCharacterDragStart(
                                                String(id),
                                                $event,
                                            )
                                        "
                                    ...
                                    >
                                        ...
                                        >
                                            <div
                                                class="text-[10px] text-white/40 uppercase"
                                            >
                                                {{ char.slot }}
                                            </div>
                                            <div
                                                class="text-xs font-bold text-white/70 mt-1"
                                            >
                                                {{ char.charId }}
                                            </div>
                                        </div>
                            ...
                            >
                                <div
                                    class="min-w-70 max-w-[70%] rounded-3xl border border-white/18 bg-black/55 px-6 py-5 text-center text-white"
                                >
                                    <div
                                    ...
                                    <div
                                        class="mt-2 text-sm font-semibold tracking-widest"
                                    >
                                        {{ stageDropHint }}
                                    </div>
                                    <div
                                    ...
                                    >
                                        ...
                                        >
                                            <span
                                                class="h-full bg-md-primary transition-all duration-200"
                                                :style="{
                                                    width:
                                                        draggedStageSlot ===
                                                        'left'
                            ...
                            <Transition name="fade">
                                <div
                                    ...
                                    class="absolute left-1/2 top-12 z-20 -translate-x-1/2 rounded-full bg-black/55 px-4 py-2 text-xs font-semibold tracking-widest text-white/85 backdrop-blur-md"
                                >
                                    {{ stageEditorMessage }}
                                </div>
                            </Transition>
                    ...
                    <Transition name="slide-panel">
                        <div
                            v-if="showEditPanel"
                            class="w-80 xl:w-96 shrink-0 bg-md-surface-container-low flex flex-col pointer-events-auto theme-dark border-l border-md-outline-variant/10"
                            @keydown.stop
                        >
                            <div
                                class="shrink-0 flex flex-col bg-transparent"
                                style="max-height: 45%"
                            >
                                <div
                                    class="shrink-0 px-4 pt-4 pb-1 flex flex-col gap-3"
                                >
                                    <input
                                        v-model="fsSearchText"
                                        class="gal-input text-xs bg-transparent"
                                        placeholder="搜索指令..."
                                    />
                                    <div class="flex items-center gap-1">
                                        <div class="flex-1">
                                            <GalDropdown
                                                ...
                                                :searchable="true"
                                                @update:model-value="
                                                    handleInsertCommandType
                                                "
                                            />
                                        </div>
                                        <button
                                            class="inline-flex items-center justify-center w-7.5 h-7.5 rounded-lg bg-white/8 text-white/60 text-sm cursor-pointer hover:bg-white/18 hover:text-white transition-colors"
                                            title="上移"
                                            @click="moveActiveCommand('up')"
                                        >
                                            ↑
                                        ...
                                        <button
                                            class="inline-flex items-center justify-center w-7.5 h-7.5 rounded-lg bg-white/8 text-white/60 text-sm cursor-pointer hover:bg-red-500/30 hover:text-red-400 transition-colors"
                                            title="删除"
                                            @click="
                                                activeCommand &&
                                                showConfirm(
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        ></span>
                                        <span
                                            class="text-[11px] font-semibold truncate flex-1"
                                            >{{ cmd.id }}</span
                                        >
                                        <span
                            ...
                            <div
                                class="flex-1 min-h-0 flex flex-col border-t border-md-outline-variant/10"
                            >
                                <div
                                ...
                                <div
                                    v-if="activeCommand"
                                    class="shrink-0 flex items-center gap-2 px-4 py-3 bg-transparent border-t border-md-outline-variant/10"
                                >
                                    <GalTag
                                        :text="typeLabel(activeCommand.type)"
                                    />
                                    <span
                                        class="text-xs text-md-on-surface font-bold tracking-wide truncate"
                                        >{{ activeCommand.id }}</span
                                    >
                                </div>

                                <div class="shrink-0 px-4 py-4 bg-transparent">
                                    <div class="flex flex-col gap-4">
                                        <div
                                            class="flex items-center justify-between gap-2"
                                        >
                                            <span
                                                class="text-[10px] font-bold uppercase tracking-widest text-md-primary"
                                                >资源库</span
                                            >
                                        </div>
                                        <GalSegmented
                                        ...
                                        >
                                            <button
                                                ...
                                                :key="asset.id"
                                                class="group overflow-hidden rounded-2xl border border-white/8 bg-white/5 text-left transition hover:bg-white/10 cursor-pointer"
                                                @click="
                                                    setBackgroundForActiveCommand(
                                                        asset.src,
                                                    )
                                                "
                                            >
                                        ...
                                        >
                                            ...
                                            <div
                                                ...
                                                class="rounded-2xl bg-white/5 px-3 py-2 text-xs text-white/65"
                                            >
                                                当前角色：{{
                                                    activeStageCharacter.charId
                                                }}
                                                · 舞台站位
                                                {{
                                            ...
                                            >
                                                <button
                                                    ...
                                                    :key="asset.id"
                                                    class="group flex min-h-28 items-end overflow-hidden rounded-2xl border border-white/8 bg-linear-to-b from-white/8 to-white/4 px-3 py-2 text-left transition hover:bg-white/10 cursor-pointer"
                                                    @click="
                                                        setStandeeForActiveCommand(
                                                            asset.src,
                                                        )
                                                    "
                                                >
                                        ...
                                        <div v-else class="flex flex-col gap-3">
                                            <div>
                                                <div
                                                    class="text-[11px] uppercase tracking-widest text-white/45 mb-2"
                                                >
                                                    BGM
                                                </div>
                                                <div
                                                ...
                                                >
                                                    <button
                                                        ...
                                                        :key="asset.id"
                                                        class="rounded-2xl border border-white/8 bg-white/5 px-3 py-2 text-left transition hover:bg-white/10 cursor-pointer"
                                                        @click="
                                                            setBgmForActiveCommand(
                                                                asset.src,
                                                            )
                                                        "
                                                    >
                                            ...
                                            >
                                                <div
                                                    class="rounded-2xl bg-white/5 px-3 py-2 min-h-28"
                                                >
                                                    <div
                                                        class="uppercase tracking-widest text-white/35 mb-2"
                                                    >
                                                        Voice
                                                    </div>
                                                    <div
                                                    ...
                                                    >
                                                        <button
                                                            ...
                                                            :key="asset.id"
                                                            class="rounded-xl border border-white/8 bg-white/5 px-2.5 py-2 text-left transition hover:bg-white/10 cursor-pointer"
                                                            @click="
                                                                setVoiceForActiveCommand(
                                                                    asset.src,
                                                                )
                                                            "
                                                        >
                                        ...
                                        <div
                                            ...
                                            class="rounded-2xl bg-md-secondary-container/40 px-3 py-2 text-xs text-white/70"
                                        >
