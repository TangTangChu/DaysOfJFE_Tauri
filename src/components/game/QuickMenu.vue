<template>
    ...
    >
        ...
        >
            <div
                v-if="showMenu"
                class="flex flex-col gap-1.5 p-3 bg-md-surface-container/85 backdrop-blur-lg text-md-on-surface border-none z-50 rounded-3xl w-27.5"
                @click.stop
            >
                <button
                    ...
                    :class="[
                        btn.isActive
                            ...
                            : 'bg-transparent text-md-on-surface-variant hover:bg-md-surface-container-highest hover:text-md-on-surface',
                    ]"
                    @click="emit('action', btn.action as any)"
                >
                    <div
                        class="absolute inset-0 bg-md-primary/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"
                    ></div>

                    <span
                        class="text-[13px] tracking-[0.15em] font-sans font-bold z-10 transition-colors duration-300"
                        :class="{ 'text-md-primary': btn.isActive }"
                        >{{ btn.label }}</span
                    >
                    <span
                    ...
                    >
                    <div
                        class="absolute right-3 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-md-primary transition-all duration-300"
                        :class="
                            btn.isActive
